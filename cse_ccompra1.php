...
$consbol = "SELECT valxb_ent, fini_sor, ffin_sor FROM entidad";
$consbol = mysqli_query($link, $consbol);
$rowbol  = mysqli_fetch_array($consbol);
...
if (isset($_POST['id_barrio'])) { $id_barrio = $_POST['id_barrio']; }

if (!empty($_POST['nrod_cli']) || !empty($codi_cli)) {
    $codi_cli_esc = mysqli_real_escape_string($link, $codi_cli);
    $tpid_cli_esc = mysqli_real_escape_string($link, $tpid_cli);
    $nrod_cli_esc = mysqli_real_escape_string($link, $nrod_cli);
    ...
    if (!empty($codi_cli)) {
        $consultacli = "SELECT codi_cli, tpid_cli, nrod_cli, exped_cli, nomb_cli, apel_cli, dire_cli, tele_cli, fnac_cli, sexo_cli, emai_cli, prof_cli, punt_cli, cliente.id_barrio, vw_barrio.descripcion
                        ...
                        WHERE codi_cli='$codi_cli_esc'";
    } else {
        $consultacli = "SELECT codi_cli, tpid_cli, nrod_cli, exped_cli, nomb_cli, apel_cli, dire_cli, tele_cli, fnac_cli, sexo_cli, emai_cli, prof_cli, punt_cli, cliente.id_barrio, vw_barrio.descripcion
                        FROM cliente
                        LEFT JOIN vw_barrio ON vw_barrio.id_barrio = cliente.id_barrio
                        WHERE tpid_cli='$tpid_cli_esc' AND nrod_cli='$nrod_cli_esc'";
    }
...
<table class='Tbl0' width='100%'>
    ...
    if (file_exists($archivo)) {
        ...
        while (($data = fgetcsv($fp, 1000, ",")) !== FALSE) {
            ...
            $campo1_esc  = mysqli_real_escape_string($link, $campo[1]);
            $consultalo  = "SELECT desc_tip AS local FROM tipo WHERE codi_tip='$campo1_esc'";
            $consultalo  = mysqli_query($link, $consultalo);
            $rowlo       = mysqli_fetch_array($consultalo);
            echo "<td class='Td2' align='left'>{$rowlo['local']}</td>";

            $campo2_esc  = mysqli_real_escape_string($link, $campo[2]);
            $consultado  = "SELECT desc_tip AS docum FROM tipo WHERE codi_tip='$campo2_esc'";
...
<table class='Tbl0' width='100%' border='0'>
    ...
    <tr>
        ...
        <td class='Td2' align='left'>
            <select name='sexo_cli' <?php echo $disp; ?>>
            <option value=''>
    ...
    <tr>
        ...
        <td class='Td2' align='left'>
            <input type='text' id="barrio" name="barrio" size='50' maxlength='50' value="<?php echo $barrio; ?>" <?php echo $disp; ?>>
    ...
    <tr>
        ...
        <td class='Td2' align='left'>
            <select name='prof_cli' <?php echo $disp; ?>>
            <option value=''>
            <?php
